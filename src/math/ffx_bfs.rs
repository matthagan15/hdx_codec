...
use super::{
    finite_field::{FFRep, FiniteField, FiniteFieldExt},
...
impl CosetRep {
    pub fn new(rep: &FFXMatrix, type_ix: usize, subgroups: &Rc<ParallelSubgroups>) -> Self {
...
impl Display for CosetRep {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut s = String::new();
        s.push_str("Type: ");
        s.push_str(&self.type_ix.to_string());
        s.push_str("\n");
        s.push_str(&self.rep.to_string());
        f.write_str(&s)
    }
}
...
impl<'de> Deserialize<'de> for CosetRep {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let mat_out = serde_json::from_str::<FFXMatrix>(matrix);
        if mat_out.is_err() {
...
impl HTypeSubgroup {
    pub fn new(prime_base: FFRep, prime_power: FFRep) -> Self {
        let mut type_zero = h_type_subgroup(0, prime_base, prime_power);
        let mut type_one = h_type_subgroup(1, prime_base, prime_power);
        let mut type_two = h_type_subgroup(2, prime_base, prime_power);
    ...
    pub fn generate_left_mul(&self, mat: &FFXMatrix) -> Vec<FFXMatrix> {
        self.gens.par_iter().map(|h| h * mat).collect()
    }

    pub fn generate_right_mul(&self, mat: &FFXMatrix) -> Vec<FFXMatrix> {
        self.gens.par_iter().map(|h| mat * h).collect()
    }
    /// pretty print of the generators
    pub fn print(&self) {
        println!("{:}", "#".repeat(70));
        println!("Type 0");
        let mut count_0 = 0;
        let mut count_1 = 0;
        let mut count_2 = 0;
        for ix in 0..self.gens.len() {
            if ix <= self.type_zero_end {
                count_0 += 1;
            } else if ix > self.type_zero_end && ix <= self.type_one_end {
                count_1 += 1;
            } else {
                count_2 += 1;
            }
            if ix == self.type_zero_end + 1 {
                println!("\nType 1\n");
            } else if ix == self.type_one_end + 1 {
                println!("\nType 2\n");
            }
            println!("{:}", self.gens[ix]);
        }
        println!("number type 0: {:}", count_0);
        println!("number type 1: {:}", count_1);
        println!("number type 2: {:}", count_2);
    }
}

fn h_type_subgroup(type_ix: usize, prime_base: FFRep, prime_power: FFRep) -> Vec<FFXMatrix> {
    let mut ret = Vec::new();
    let dim = 3;
    let id = FFXMatrix::id(dim, prime_base, prime_power);
    let mut row_ix = type_ix as i32 - 1;
    while row_ix <= 0 {
        row_ix += dim as i32;
    }
    row_ix %= dim as i32;
    for a in 0..prime_base {
        let mut tmp = id.clone();
        let mut x = FiniteFieldExt::get_basis(1, prime_base, prime_power);
        x = &x * a as i32;
        tmp.set_entry(row_ix as usize, type_ix, x);
        ret.push(tmp);
    }
    ret
}
...
impl GroupBFS {
    pub fn new(directory: &Path, prime_base: FFRep, prime_power: FFRep) -> Self {
        ...
        if previously_cached.is_some() {
            ...
            pbuf.push(directory);
            let mut ret = Self {
                subgroups: ParallelSubgroups::new(3, prime_base, prime_power),
                h_gens: HTypeSubgroup::new(prime_base, prime_power),
                prime_base,
                prime_power,
            ...
            let e = FFXMatrix::id(3, prime_base, prime_power);
    ...
    pub fn get_hgraph_file_path(&self) -> PathBuf {
        ...
        filename.push_str(&self.prime_base.to_string());
        filename.push_str("_dim_");
        filename.push_str("3");
        filename.push_str("_deg_");
        filename.push_str(&self.prime_power.to_string());
    ...
    fn save_hgraph(&self) {
        let file_path = self.get_hgraph_file_path();
    ...
    pub fn bfs(&mut self, max_num_steps: usize) {
        let p = self.prime_base;
        let dim = 3;
        let deg = self.prime_power;
        ...
        while self.frontier.is_empty() == false && counter < max_num_steps {
            ...
            if self.num_matrices_completed % 1 == 0 {
                ...
                println!("Frontier length: {:}", self.frontier.len());
                println!("Visited length: {:}", self.visited.len());
                println!("{:}", "$".repeat(65));
            }
        }
    ...
    fn step(&mut self) {
        ...
        println!("_______VISITED_______");
        for v in self.visited.iter() {
            println!("dist={:}, mat={:}", v.distance, v.mat.to_string());
        }
        println!("_______VISITED_______");
        ...
        for neighbor in neighbors {
            ...
            println!(
                "dist={:}\nmat= {:}\ncontains={:}",
                neighbor_bfs.distance,
                neighbor_bfs.mat.to_string(),
                self.visited.contains(&neighbor_bfs)
            );
            println!("****neighbor****");
...
struct ParallelSubgroups {
    generators: Vec<FFXMatrix>,
    type_zero_end: usize,
    type_one_end: usize,
}

impl ParallelSubgroups {
    pub fn new(dim: usize, prime_base: FFRep, prime_power: FFRep) -> Self {
        let id = FFXMatrix::id(dim, prime_base, prime_power);
        let mut type_to_gens = HashMap::new();

        for type_ix in 0..dim {
            let ret_type_i: &mut HashSet<FFXMatrix> = type_to_gens.entry(type_ix).or_default();
                ...
                for col_ix in 0..dim {
                    ...
                    if deg > 0 {
                        ...
                        for matrix in matrices.iter() {
                            for c in 1..prime_base {
                                let mut new_matrix = matrix.clone();
                                let monomial =
                                    &FiniteFieldExt::get_basis(deg, prime_base, prime_power)
                                        * (c as i32);
    ...
    pub fn print(&self) {
        ...
        let mut count_1 = 0;
        let mut count_2 = 0;
        for ix in 0..self.generators.len() {
            if ix <= self.type_zero_end {
                count_0 += 1;
            } else if ix > self.type_zero_end && ix <= self.type_one_end {
                count_1 += 1;
    ...
    pub fn get_coset(&self, coset_rep: &FFXMatrix, type_ix: usize) -> Vec<FFXMatrix> {
        if type_ix == 0 {
            let subs = &self.generators[0..=self.type_zero_end];
            let mut coset: Vec<FFXMatrix> = subs.par_iter().map(|k| coset_rep * k).collect();
            coset.sort();
            coset
        } else if type_ix == 1 {
    ...
    pub fn get_all_cosets(
        &self,
        rep: &FFXMatrix,
    ) -> (Vec<FFXMatrix>, Vec<FFXMatrix>, Vec<FFXMatrix>) {
        let v: Vec<FFXMatrix> = self.generators.par_iter().map(|k| rep * k).collect();
    ...
    pub fn get_canonical_rep(&self, rep: &FFXMatrix, type_ix: usize) -> FFXMatrix {
        let coset = self.get_coset(rep, type_ix);
        coset[0].clone()
    }

    pub fn get_coset_reps(&self, mat: &FFXMatrix) -> (CosetRep, CosetRep, CosetRep) {
...
mod tests {
    ...
    use crate::math::{
        ffmatrix::FFXMatrix, polymatrix::PolyMatrix, polynomial::FiniteFieldPolynomial,
    };

    use super::{GroupBFS, GroupBFSNode, HTypeSubgroup, ParallelSubgroups, Subgroups};
    ...
    fn test_group_bfs_manager() {
        ...
        let mut bfs_manager = GroupBFS::new(&directory, 3, 2);
    ...
    fn test_bfs_nodes() {
        ...
        let primitive_coeffs = [(2, (1, p).into()), (1, (2, p).into()), (0, (2, p).into())];
        let q = FiniteFieldPolynomial::from(&primitive_coeffs[..]);
        let e = FFXMatrix::id(3, 3, 2);
        let sg = ParallelSubgroups::new(3, 3, 2);
    ...
    #[test]
    fn test_new_short_walk() {
        let dir = PathBuf::from("/Users/matt/repos/qec/tmp");
        let p = 3_u32;
        let primitive_coeffs = [(2, (1, p).into()), (1, (2, p).into()), (0, (2, p).into())];
        let q = FiniteFieldPolynomial::from(&primitive_coeffs[..]);
        let mut bfs = GroupBFS::new(&dir, 3, 2);
        bfs.bfs((2 as usize).pow(20));
        println!("graph: {:}", bfs.hg);
    }

    #[test]
    fn test_subgroup_gens() {
        let subs = ParallelSubgroups::new(3, 3, 2);
        let h_type_subs = HTypeSubgroup::new(3, 2);
        let id = FFXMatrix::id(3, 3, 2);
        subs.print();
        h_type_subs.print();
        ...
        for o in out.iter() {
            println!("{:}", o);
            let (r0, r1, r2) = subs.get_coset_reps(o);
            println!("__reps__\n{:}\n{:}\n{:}", r0, r1, r2)
        }
